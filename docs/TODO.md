...
- **Bug Fix**: 2025-12-30 23:40 KST — Fixed bootstrap.sh path configuration issues: (1) Changed TERRAFORM_DIR from `infra/environments/${ENVIRONMENT}` to `infra/` to match actual directory structure, (2) Added ENV_VARS_FILE variable pointing to `env/${ENVIRONMENT}/terraform.tfvars`, (3) Updated terraform plan command to use `-var-file="${ENV_VARS_FILE}"`, (4) Added validation check for ENV_VARS_FILE existence. Root cause: Script assumed Terraform files were in environment-specific subdirectories, but actual implementation has Terraform in `infra/` root with environment configs in separate `env/` directory.
- **Bug Fix**: 2025-12-30 23:45 KST — Removed Helm chart dependency .tgz files from Git tracking to comply with GitOps principles: (1) Created comprehensive .gitignore excluding platform/*/charts/*.tgz and Chart.lock files, (2) Removed 4 .tgz files from Git cache (argo-cd-5.51.0.tgz, airflow-1.12.0.tgz, trino-0.15.0.tgz, kube-prometheus-stack-55.0.0.tgz), (3) Removed 4 Chart.lock files from Git tracking, (4) Updated platform/README.md with Helm dependency management section explaining GitOps principles, (5) Updated platform/argocd/README.md with note about automatic dependency download. Root cause: Binary .tgz files were committed to Git, violating "Git as single source of truth" principle. Dependencies should be declared in Chart.yaml and downloaded via `helm dependency update`.
